...
namespace Bryllite.Cryptography.Aes
{
    public class Aes256
    {
        ...
        {
            this.key = KeccakProvider.Hash256(key);
            this.iv = KeccakProvider.Hash128(iv);
        }

        public Aes256(byte[] key) : this(key, key)
        {
        }
        ...
        {
            return new Aes256(key, iv).Encrypt(plain);
        }

        public static byte[] Encrypt(byte[] key, byte[] plain)
        {
            return new Aes256(key).Encrypt(plain);
        }

        public static byte[] Decrypt(byte[] key, byte[] iv, byte[] encrypted)
        {
            return new Aes256(key, iv).Decrypt(encrypted);
        }

        public static byte[] Decrypt(byte[] key, byte[] encrypted)
        {
            return new Aes256(key).Decrypt(encrypted);
        }

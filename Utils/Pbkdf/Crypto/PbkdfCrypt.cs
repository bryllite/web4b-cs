...
namespace Bryllite.Utils.Pbkdf.Crypto
{
    class PbkdfCrypt
    {
        internal static byte[] GenerateDerivedScryptKey(string password, byte[] salt, int n, int r, int p, int dkLen, bool checkRandN = false)
        {
            if (checkRandN)
            {
                ...
                if (r == 1 && n >= 65536)
                    throw new ArgumentOutOfRangeException("Cost parameter N must be > 1 and < 65536");
            }
        ...
        {
            var message = new byte[16 + cipherText.Length];
            Array.Copy(derivedKey, 16, message, 0, 16);
            Array.Copy(cipherText, 0, message, 16, cipherText.Length);

            return KeccakProvider.Hash256(message);
        }
        ...
        internal static void ValidateMac(byte[] mac, byte[] cipherText, byte[] derivedKey)
        {
            ...
            if (!generatedMac.SequenceEqual(mac))
                throw new ArgumentException("Cannot derive the same mac as the one provided from the cipher and derived key");
        }

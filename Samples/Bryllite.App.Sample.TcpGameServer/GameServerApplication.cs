...
{
    public class GameServerApplication : AppBase
    {
        // game db
        ...
        public readonly BrylliteApiService ApiService;

        public GameServerApplication(string[] args) : base(args)
        {
            ...
            ApiService = new BrylliteApiService(
                config["bridge"].Value<string>("url"), 
                config["game"].Value<string>("key"), 
                config["coinbox"].Value<string>("address")
            );
        }
